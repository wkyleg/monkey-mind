...
export interface GameEvents {
  ...
  // Combat
  'enemy:spawn': { type: string; x: number; y: number; actVisual?: string };
  'enemy:damage': { id: string; amount: number };
  'enemy:death': { id: string; type: string; position: { x: number; y: number }; actVisual?: string };
  ...
  'weapon:fire': undefined;

  // Powerups
  'powerup:spawn': { id: string; type: string };
  'powerup:collect': { type: string; effect: string };
  'powerup:expire': { type: string };
  'powerup:time_slow': { active: boolean; factor: number };
  'powerup:magnet': { active: boolean; range: number };
