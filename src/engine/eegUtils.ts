...
/**
 * Normalize EEG data to channel-major layout (channels × samples).
 * If the outer dimension matches expectedChannels, treat as already channel-major.
 * Otherwise, transpose from sample-major (samples × channels).
 */
export function asChannelMajor(data: number[][], expectedChannels?: number): number[][] {
  ...
  // Heuristic: small inner dimension + large outer → sample-major
