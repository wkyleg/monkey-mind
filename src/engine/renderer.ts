...
export class Renderer {
  ...
  gradientBackground(colors: string[], angle: number = 0): void {
    ...
    const gradient = this.ctx.createLinearGradient(cx - dx, cy - dy, cx + dx, cy + dy);
  ...
  /**
   * Clean monospace HUD text - with outline for readability on busy backgrounds
   */
  hudText(
    text: string,
    x: number,
    ...
    this.ctx.font = `bold ${size}px 'SF Mono', Consolas, 'Liberation Mono', monospace`;
    this.ctx.textAlign = align;
   ...
   */
  drawAngularFrame(x: number, y: number, w: number, h: number, color: string, cornerSize: number = 15): void {
    this.ctx.strokeStyle = color;
