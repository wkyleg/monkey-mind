...
export class DifficultySystem {
  ...
  update(dt: number): void {
    ...
    const hitRate =
      this.recentHits + this.recentMisses > 0 ? this.recentHits / (this.recentHits + this.recentMisses) : 0.5;
    ...
    this.playerStress = lerp(this.playerStress, stressFromHits * 0.6 + stressFromReaction * 0.4, dt * 0.5);

    // Performance score based on hit rate and survival
    const survivalBonus = Math.min(1, this.playTime / 60);
    this.performanceScore = lerp(this.performanceScore, hitRate * 0.7 + survivalBonus * 0.3, dt * 0.3);
