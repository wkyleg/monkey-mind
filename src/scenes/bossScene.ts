...
import { Boss, BossFactory } from '../gameplay/bosses';
...
export class BossScene extends Scene {
  ...
  update(dt: number, intent: PlayerIntent): void {
    ...
    if (intent.cancel && !this.gameOver && !this.victory) {
      this.game.getScenes().push('pause');
      return;
    }
  ...
  private renderBossHealthBar(renderer: Renderer, width: number): void {
    ...
    // Health fill
    const healthColor = healthPercent > 0.5 ? CONFIG.COLORS.DANGER : 
                        healthPercent > 0.25 ? CONFIG.COLORS.ACCENT : '#ff0000';
